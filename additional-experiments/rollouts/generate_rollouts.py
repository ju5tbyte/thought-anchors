...
parser.add_argument('-ds', '--dataset', type=str, default='math', choices=['math', 'strategyqa'], help='Dataset to use')
...
# Load dataset config (determines prompts, answer extraction, and answer checking)
...
async def generate_base_solution(problem: Dict, temperature: float = 0.6) -> Dict:
    ...
    """
    # Build prompt via dataset config
    ...
    for attempt in range(max_retries):
        try:
            ...
            answer = dataset_config["extract_answer"](solution_text)
            is_correct = False

            if problem.get('gt_answer') and answer:
                is_correct = dataset_config["check_answer"](answer, problem['gt_answer'])
...
async def generate_rollout(problem: Dict, chunk_text: str, full_cot_prefix: str, temperature: float = 0.7, rollout_type: str = 'default') -> Dict:
    ...
    # Build prompt via dataset config
    prompt = dataset_config["build_rollout_prompt"](problem, prefix_without_chunk)

    if rollout_type == 'forced_answer' and args.dataset == 'math':
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Extract answer and check correctness
            text_for_answer = f"{prompt}{rollout_text}" if rollout_type == 'forced_answer' else rollout_text
            answer = dataset_config["extract_answer"](text_for_answer)
            is_correct = False
    ...
    if base_solution_file.exists() and not args.force:
        with open(base_solution_file, 'r', encoding='utf-8') as f:
            ...
            if not args.skip_recalculate and 'solution' in base_solution:
                answer = dataset_config["extract_answer"](base_solution['solution'])
                is_correct = False
                ...
                if not args.skip_recalculate:
                    ...
                    for rollout in existing_solutions:
                        if 'rollout' in rollout and 'error' not in rollout:
                            text_for_answer = rollout['full_cot'] if args.rollout_type == 'forced_answer' else rollout['rollout']
                            answer = dataset_config["extract_answer"](text_for_answer)
                            is_correct = False
        ...
        if num_rollouts_needed > 0:
            ...
            if args.provider == "Local":
                ...
                for _ in tqdm(range(num_rollouts_needed), desc="Generating rollouts"):
                    ...
                    if args.rollout_type == 'forced_answer' and args.dataset == 'math':
                        prompt += "\n</think>\n\nTherefore, the final answers is \\boxed{"
                ...
                for i, result in enumerate(batch_results):
                    ...
                    # Extract answer and check correctness
                    prompt = prompts[i]
                    text_for_answer = f"{prompt}{rollout_text}" if args.rollout_type == 'forced_answer' else rollout_text
                    answer = dataset_config["extract_answer"](text_for_answer)
                    is_correct = False

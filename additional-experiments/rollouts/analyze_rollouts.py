...
parser.add_argument(
    ...
    help="Dataset being analyzed (determines DAG prompt and nickname style)",
)

args = parser.parse_args()

# Select DAG prompt based on dataset
_active_dag_prompt = DATASET_DAG_PROMPTS.get(args.dataset, DAG_PROMPT)
# print(f"{args=}")
# quit()
...
def generate_problem_nickname(problem_text: str) -> str:
    ...
    """
    # Create a prompt to get a concise nickname (adapts to dataset type)
    _is_math = args.dataset == "math"
    if _is_math:
    ...
    """
        ...
        prompt = f"""Please provide a 2-4 word maximum nickname for this reasoning question that captures its essence. Focus on the main topic or domain.

    Examples:
    - "Amazon river length" (for questions about river lengths)
    - "Moon landing year" (for questions about historical events)
    - "Spider leg count" (for questions about animal biology)
    ...
    _default_nickname = "math problem" if _is_math else "reasoning question"
...
def label_chunk(problem_text: str, chunks: List[str], chunk_idx: int) -> Dict:
    ...
    # Format the DAG prompt with the problem and chunks (dataset-specific)
    formatted_prompt = _active_dag_prompt.format(
...
def analyze_problem(
    ...
    if (
        ...
        try:
            ...
            problem["nickname"] = "math problem" if args.dataset == "math" else "reasoning question"
            ...
            for chunk in chunks_need_summary:
                ...
                if chunk_text:
                    try:
                        ...
                        print(
                            f"Error generating summary for chunk {chunk.get('chunk_idx')}: {e}"
                        )
